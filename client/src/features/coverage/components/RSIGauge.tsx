...
export default function RSIGauge({ rsi, language = "tr" }: RSIGaugeProps) {
  ...
  const zoneColor = () => {
    if (clamped < 30) return "text-rose-400";
    if (clamped > 70) return "text-emerald-400";
    return "text-amber-400";
  };

  const zoneLabel = () => {
    if (clamped < 30) return t("Asiri Satti (Oversold)", "Oversold");
    if (clamped > 70) return t("Asiri Alimli (Overbought)", "Overbought");
    return t("Notr", "Neutral");
  };
