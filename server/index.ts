...
function normalizePortfolioAssetType(value: unknown): PortfolioAssetType {
...
function getPortfolioPositionCostBasis(position: PortfolioPositionRecord) {
  const multiplier = position.assetType === "stock" ? 1 : 100;
  return roundMoney(
    Math.max(0, position.quantity) * Math.max(0, position.entryPrice) * multiplier
  );
}
...
function resolveWeeklyPortfolioBias(entry: WeeklyReportEntry) {
  if (entry.directionalBias === "Bullish") {
...
function resolveMomentumPortfolioBias(entry: MomentumReportEntry) {
  const normalizedSignal = normalizeString(entry.signal).toUpperCase();
  if (
    normalizedSignal.includes("BUY") ||
    normalizedSignal.includes("BULL") ||
    entry.score >= 65
  ) {
    return "bullish" as const;
  }
  ...
  if (
    ...
    normalizedSignal.includes("SELL") ||
    normalizedSignal.includes("BEAR") ||
    entry.score <= 35
  ) {
    return "bearish" as const;
  }
...
function buildPortfolioContextSummary(): PortfolioContextSummary {
  const weeklyReports = getViewerWeeklyReports();
  const momentumReport = billingStore.getLatestPublishedMomentumReport();
  const dailyReports = getViewerDailyReports(3);

  return {
    earningStrategy: weeklyReports.map(
      report =>
        ({
          module: "earning_strategy",
          reportId: report.id,
          title: report.title,
          asOfDate: report.analysisDate,
          coverageCount: report.content.entries.length,
          subtitle: `${report.weekStart} -> ${report.weekEnd}`,
        }) satisfies PortfolioModuleSummary
    ),
    momentum: momentumReport
      ? [
          {
            module: "momentum",
            reportId: momentumReport.id,
            title: momentumReport.title,
            asOfDate: momentumReport.reportDate,
            coverageCount: momentumReport.content.featuredEntries.length,
            subtitle:
              normalizeString(momentumReport.content.scanTime) ||
              momentumReport.content.scannerUniverse,
          } satisfies PortfolioModuleSummary,
        ]
      : [],
    daily: dailyReports.map(
      report =>
        ({
          module: "daily",
          reportId: report.id,
          ...
          coverageCount: report.content.tickerUniverse.length,
          subtitle: report.content.headline,
        }) satisfies PortfolioModuleSummary
    ),
  };
}

function analyzePortfolioPositions(
  positions: PortfolioPositionRecord[],
  requestedAccountSize?: number
): PortfolioAnalysisRecord {
  const weeklyReports = getViewerWeeklyReports();
  const momentumReport = billingStore.getLatestPublishedMomentumReport();
  const dailyReports = getViewerDailyReports(5);
  const context = buildPortfolioContextSummary();

  const findWeeklyMatch = (ticker: string) => {
    for (const report of weeklyReports) {
      const entry = report.content.entries.find(item => item.ticker === ticker);
      if (entry) {
        return { report, entry };
      }
    }
  ...
  const findMomentumMatch = (ticker: string) => {
    if (!momentumReport) {
      return null;
    }

    const entry = momentumReport.content.featuredEntries.find(item => item.ticker === ticker);
    return entry ? { report: momentumReport, entry } : null;
  };

  const findDailyMatch = (ticker: string) => {
    for (const report of dailyReports) {
      if (report.content.tickerUniverse.includes(ticker)) {
        return { report };
      }
    }
  ...
  const insights = positions.map(position => {
    const ticker = position.ticker.toUpperCase();
    const weeklyMatch = findWeeklyMatch(ticker);
    const momentumMatch = findMomentumMatch(ticker);
    const dailyMatch = findDailyMatch(ticker);
    const moduleMatches: PortfolioModuleMatch[] = [];
    const signals: string[] = [];
    const warnings: string[] = [];
    const costBasis = getPortfolioPositionCostBasis(position);
    const optionPosition = position.assetType !== "stock";
    const bullishPosition = position.assetType !== "put";

    const weeklySignal = weeklyMatch
      ? weeklyMatch.entry.momentumScore * 0.45 +
        weeklyMatch.entry.ivCrushScore * 0.35 +
        weeklyMatch.entry.beatRate * 0.2
      : null;
    const momentumSignal = momentumMatch
      ? momentumMatch.entry.score * 0.65 + momentumMatch.entry.confidenceScore * 0.35
      : null;
    const dailySignal = dailyMatch ? 68 : null;

    let weightedSignal = 0;
    let totalWeight = 0;
    if (weeklySignal !== null) {
      weightedSignal += weeklySignal * 0.46;
      totalWeight += 0.46;
    }
    if (momentumSignal !== null) {
      weightedSignal += momentumSignal * 0.34;
      totalWeight += 0.34;
    }
    if (dailySignal !== null) {
      weightedSignal += dailySignal * 0.2;
      totalWeight += 0.2;
    }

    let convictionScore =
      totalWeight > 0 ? weightedSignal / totalWeight : optionPosition ? 34 : 38;
    let riskScore = optionPosition ? 58 : 34;

    if (weeklyMatch) {
      const weeklyBias = resolveWeeklyPortfolioBias(weeklyMatch.entry);
      moduleMatches.push({
        module: "earning_strategy",
        reportId: weeklyMatch.report.id,
        title: weeklyMatch.report.title,
        asOfDate: weeklyMatch.report.analysisDate,
        bias: weeklyBias,
        signalScore: clampScore(weeklySignal || 0),
        note: `${weeklyMatch.entry.recommendedStrategy} | Momentum ${weeklyMatch.entry.momentumScore} | IV ${weeklyMatch.entry.ivCrushScore}`,
      });

      signals.push(
        `Earning Strategy: ${weeklyMatch.entry.recommendedStrategy} (${weeklyMatch.entry.directionalBias})`
      );
      signals.push(
        `Beat rate ${weeklyMatch.entry.beatRate}% ve momentum ${weeklyMatch.entry.momentumScore}`
      );

      riskScore += weeklyMatch.entry.earningsMissRisk * 0.28;
      riskScore += weeklyMatch.entry.gapRisk * 0.24;

      if (weeklyMatch.entry.riskLevel === "VERY_HIGH") {
        riskScore += 10;
      } else if (weeklyMatch.entry.riskLevel === "HIGH") {
        riskScore += 6;
      }

      if (
        (bullishPosition && weeklyBias === "bearish") ||
        (!bullishPosition && weeklyBias === "bullish")
      ) {
        convictionScore -= 18;
        riskScore += 12;
        warnings.push("Weekly directional bias mevcut pozisyon yonu ile uyumsuz.");
      } else if (weeklyBias === "neutral") {
        convictionScore -= 6;
      }

      const daysToEarnings = getDaysUntilIsoDate(weeklyMatch.entry.earningsDate);
      if (daysToEarnings !== null && daysToEarnings <= 7) {
        warnings.push(`Earnings etkinligi ${daysToEarnings} gun icinde.`);
      }

      if (optionPosition && weeklyMatch.entry.expectedIVCrush >= 28) {
        warnings.push(
          `Beklenen IV crush ${weeklyMatch.entry.expectedIVCrush}% seviyesinde; event icine tam boy tasimayin.`
        );
        riskScore += 8;
      }
    } else {
      warnings.push("Earning Strategy modulu icinde aktif coverage bulunmuyor.");
    }

    if (momentumMatch) {
      const momentumBias = resolveMomentumPortfolioBias(momentumMatch.entry);
      moduleMatches.push({
        module: "momentum",
        reportId: momentumMatch.report.id,
        title: momentumMatch.report.title,
        asOfDate: momentumMatch.report.reportDate,
        bias: momentumBias,
        signalScore: clampScore(momentumSignal || 0),
        note: `${momentumMatch.entry.signal} | Score ${momentumMatch.entry.score} | Confidence ${momentumMatch.entry.confidenceScore}`,
      });

      signals.push(
        `Momentum: ${momentumMatch.entry.signal} skoru ${momentumMatch.entry.score}`
      );

      if (
        (bullishPosition && momentumBias === "bearish") ||
        (!bullishPosition && momentumBias === "bullish")
      ) {
        convictionScore -= 14;
        riskScore += 8;
        warnings.push("Momentum snapshot mevcut pozisyona ters yone isaret ediyor.");
      }

      if (momentumMatch.entry.confidenceScore < 55) {
        warnings.push("Momentum confidence dusuk; sizing konservatif kalmali.");
        riskScore += 4;
      }
    } else {
      warnings.push("Momentum snapshot icinde eslesen yayinda ticker yok.");
    }

    if (dailyMatch) {
      moduleMatches.push({
        module: "daily",
        reportId: dailyMatch.report.id,
        title: dailyMatch.report.title,
        asOfDate: dailyMatch.report.reportDate,
        bias: "context",
        signalScore: 68,
        note: dailyMatch.report.content.headline,
      });
      signals.push(`Daily: ${dailyMatch.report.title} ticker coverage icinde.`);
    } else {
      warnings.push("Daily rapor coverage'i disinda; tema destegi zayif.");
    }

    const daysToExpiry = getDaysUntilIsoDate(position.expiryDate);
    if (optionPosition && daysToExpiry !== null) {
      if (daysToExpiry <= 14) {
        warnings.push(`Opsiyon vadesi ${daysToExpiry} gun icinde; gamma riski yuksek.`);
        riskScore += 10;
      } else if (daysToExpiry <= 30) {
        warnings.push(`Opsiyon vadesi ${daysToExpiry} gun icinde; zaman degeri hizli eriyebilir.`);
        riskScore += 5;
      }
    }

    if (optionPosition && !position.expiryDate) {
      warnings.push("Opsiyon vadesi girilmedi; risk tahmini konservatif tutuldu.");
      riskScore += 5;
    }

    if (optionPosition && typeof position.strikePrice !== "number") {
      warnings.push("Strike bilgisi eksik; setup kalitesi tam skorlanamadi.");
      convictionScore -= 4;
    }

    convictionScore = clampScore(convictionScore);
    riskScore = clampScore(riskScore);

    let action: PortfolioStrategyAction = "watch";
    if (convictionScore >= 78 && riskScore <= 52) {
      action = "add";
    } else if (convictionScore >= 62 && riskScore <= 68) {
      action = "hold";
    } else if (riskScore >= 82) {
      action = optionPosition ? "exit" : "hedge";
    } else if (convictionScore <= 36) {
      action = optionPosition ? "exit" : "trim";
    } else if (riskScore >= 70) {
      action = optionPosition ? "trim" : "hedge";
    }

    const strategyByAction: Record<PortfolioStrategyAction, string> = {
      add: optionPosition
        ? "Setup halen destekli. Yeni kontrati ayni yone kucuk boyutta ekle ve tercihen daha gec vade veya spread ile risk tanimla."
        : "Mevcut temayi koru. Ilaveyi pullback uzerinden kademeli yap, event haftasina girerken stop seviyesini netlestir.",
      hold: optionPosition
        ? "Pozisyon korunabilir; ancak earnings oncesi IV genislemesinde parcali kar alma plani tut."
        : "Core pozisyon korunabilir. Yeni risk almadan once momentum teyidi ve daily akisi izle.",
      trim: optionPosition
        ? "Premium maruziyetini parcali azalt. Event icine tam boy girmek yerine kar realize edip daha iyi vade/strike dusun."
        : "Pozisyon agirligini dusur ve nakdi daha iyi coverage alan isimlere kaydir.",
      hedge: optionPosition
        ? "Kontrati spreade cevir veya kismi realize et. Delta ve vega maruziyetini tek evente yigma."
        : "Pozisyonu korurken koruyucu put veya covered call ile delta riskini azalt.",
      exit: optionPosition
        ? "Kalan premiumu korumak icin cikis daha mantikli. Yeterli moduler destek yok veya risk asiri yuksek."
        : "Veri destegi zayif. Pozisyonu kapatip daha guclu coverage alan setup'lara don.",
      watch:
        "Yeni ekleme yapma. Bir sonraki momentum ve daily guncellemesi gelene kadar sadece izleme modunda kal.",
    };

    const thesisParts = [
      weeklyMatch
        ? `${weeklyMatch.entry.directionalBias} biasli weekly setup`
        : "weekly coverage disi",
      momentumMatch
        ? `momentum skoru ${momentumMatch.entry.score}`
        : "momentum snapshot disi",
      dailyMatch ? "daily tema icinde" : "daily tema disi",
    ];

    const estimatedRiskFactor = optionPosition
      ? Math.min(0.95, 0.6 + riskScore / 200)
      : Math.min(0.45, 0.12 + riskScore / 220);
    const estimatedRisk = roundMoney(costBasis * estimatedRiskFactor);
    ...
    return {
      ...
      riskScore,
      action,
      thesis: `${ticker}: ${thesisParts.join(", ")}.`,
      strategy: strategyByAction[action],
      costBasis,
      estimatedRisk,
      moduleMatches,
      signals,
      warnings,
    } satisfies PortfolioPositionInsight;
  });

  const totalCostBasis = roundMoney(
    insights.reduce((sum, insight) => sum + insight.costBasis, 0)
  );
  const estimatedPortfolioRisk = roundMoney(
    insights.reduce((sum, insight) => sum + insight.estimatedRisk, 0)
  );
  const fallbackAccountSize = Math.max(totalCostBasis * 2.5, 30_000);
  const accountSize =
    typeof requestedAccountSize === "number" && requestedAccountSize > 0
      ? requestedAccountSize
      : fallbackAccountSize;
  const heatPct = roundMoney((estimatedPortfolioRisk / Math.max(accountSize, 1)) * 100);
  const canAddRisk = heatPct < 5;

  const weightedScoreBase = insights.reduce((sum, insight) => {
    const weight = insight.costBasis > 0 ? insight.costBasis : 1;
    return sum + insight.convictionScore * weight;
  }, 0);
  const weightTotal = insights.reduce(
    (sum, insight) => sum + (insight.costBasis > 0 ? insight.costBasis : 1),
    0
  );
  const overallScore = clampScore(
    weightTotal > 0 ? weightedScoreBase / weightTotal : 0
  );
  const averageRisk = clampScore(
    insights.reduce((sum, insight) => sum + insight.riskScore, 0) /
      Math.max(insights.length, 1)
  );

  let posture: PortfolioPosture = "balanced";
  if (heatPct >= 5 || averageRisk >= 72) {
    posture = "defensive";
  } else if (overallScore >= 72 && heatPct <= 3.2) {
    posture = "aggressive";
  }

  const prioritized = [...insights].sort((left, right) => {
    const actionPriority: Record<PortfolioStrategyAction, number> = {
      exit: 6,
      hedge: 5,
      trim: 4,
      watch: 3,
  ...
  const nextActions = prioritized.slice(0, 3).map(insight => {
    const base = `${insight.position.ticker}: ${insight.action.toUpperCase()}`;
    if (insight.action === "add") {
      return `${base} - ek risk yalniz kademeli ve teyitli pullback ile alin.`;
    }

    if (insight.action === "hold") {
      return `${base} - core korunabilir, yeni alimi aceleye getirme.`;
    }

    if (insight.action === "hedge") {
      return `${base} - event oncesi hedge veya spread dusun.`;
    }

    if (insight.action === "trim") {
      return `${base} - agirligi azaltip nakdi daily coverage alan isimlere kaydir.`;
    }

    if (insight.action === "exit") {
      return `${base} - veri destegi zayif, premium veya sermayeyi koru.`;
    }

    return `${base} - yeni risk ekleme, sonraki raporu bekle.`;
  });

  if (heatPct >= 5) {
    nextActions.unshift(
      "Portfoy isi %5 limitinin ustunde. Yeni pozisyon acmadan once risk butcesini dusur."
    );
  }

  const summary =
    posture === "defensive"
      ? "Portfoy savunmaci moda gecmeli. Mevcut maruziyet diger uc modulden gelen sinyallere gore fazla sicak."
      : posture === "aggressive"
        ? "Portfoy genel olarak destekli. Uc modulden gelen sinyaller hizli ekleme yerine kontrollu agresifligi destekliyor."
        : "Portfoy dengeli fakat secici olunmali. En guclu coverage alan pozisyonlari tutup zayif kalanlari filtrelemek daha dogru.";

  return {
    generatedAt: new Date().toISOString(),
    accountSize: roundMoney(accountSize),
    totalCostBasis,
    estimatedPortfolioRisk,
    heatPct,
    canAddRisk,
    posture,
    overallScore,
    ...
    context,
    positions: insights,
  } satisfies PortfolioAnalysisRecord;
}
...
async function startServer() {
  ...
  app.get("/api/portfolio", (req, res) => {
    setPrivateNoStore(res);
    const actor = requireSubscribedActor(req, res);
    if (!actor) {
      return;
    }

    res.status(200).json({
      positions: billingStore.listPortfolioPositionsByUserId(actor.id),
      context: buildPortfolioContextSummary(),
    });
  });
  ...
  app.post("/api/portfolio", (req, res) => {
    ...
    const body = (req.body ?? {}) as PortfolioPositionUpsertRequestBody;
    const payload = extractObjectRecord(body.position);
    if (!payload) {
      res.status(400).json({ error: "Pozisyon payload gerekli." });
      return;
    }

    const requestedId = normalizeString(payload.id);
    const existing = requestedId
      ? billingStore.getPortfolioPositionById(requestedId)
      : null;
    if (requestedId && (!existing || existing.userId !== actor.id)) {
      res.status(404).json({ error: "Portfoy pozisyonu bulunamadi." });
      return;
    }
    ...
    const assetType = normalizePortfolioAssetType(payload.assetType);
    const quantity = normalizeNumber(payload.quantity, 0);
    const entryPrice = normalizeNumber(payload.entryPrice, 0);
    const entryDate =
      normalizeOptionalIsoDate(payload.entryDate) ||
      new Date().toISOString().slice(0, 10);
    const strikePriceValue = normalizeNumber(payload.strikePrice, 0);
    const strikePrice =
      assetType === "stock" || strikePriceValue <= 0 ? undefined : strikePriceValue;
    const expiryDate = normalizeOptionalIsoDate(payload.expiryDate);

    if (quantity <= 0 || entryPrice <= 0) {
      res.status(400).json({
        error: "Adet ve maliyet pozitif sayi olmali.",
      });
      return;
    }

    if (assetType !== "stock" && (!strikePrice || !expiryDate)) {
      res.status(400).json({
        error: "Opsiyon pozisyonlari icin strike ve vade gerekli.",
      });
      return;
    }
    ...
    billingStore.upsertPortfolioPosition(record);
  ...
  app.delete("/api/portfolio/:id", (req, res) => {
    ...
    const positionId = normalizeString(req.params.id);
    if (!positionId) {
      res.status(400).json({ error: "Pozisyon id gerekli." });
      return;
    }

    const existing = billingStore.getPortfolioPositionById(positionId);
    if (!existing || existing.userId !== actor.id) {
      res.status(404).json({ error: "Portfoy pozisyonu bulunamadi." });
      return;
    }

    billingStore.deletePortfolioPosition(actor.id, positionId);
    res.status(200).json({
      positions: billingStore.listPortfolioPositionsByUserId(actor.id),
      context: buildPortfolioContextSummary(),
    });
  ...
  app.post("/api/portfolio/analyze", (req, res) => {
    ...
    const positions = billingStore.listPortfolioPositionsByUserId(actor.id);
    if (!positions.length) {
      res.status(400).json({
        error: "Portfoy bos. Once hisse veya opsiyon ekleyin.",
      });
      return;
    }

    const body = (req.body ?? {}) as PortfolioAnalyzeRequestBody;
    const accountSize = normalizeNumber(body.accountSize, 0);
    const analysis = analyzePortfolioPositions(
      positions,
      accountSize > 0 ? accountSize : undefined
    );
