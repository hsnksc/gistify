...
import type { StockResult, ScoreExplanation, ConfidenceBreakdown, RankingInfo } from "@/types/scanner";
...
// ═══════════════════════════════════════════════════════════════════════════════
// v4.2: ÇİFT YÖNLÜ MOTOR — Bear Skoru + Persistence Skoru
// ═══════════════════════════════════════════════════════════════════════════════

import type { BearScoreResult, PersistenceResult } from "@/types/scanner";

/**
 * Bear (Put) Skoru Hesaplama
 * Bullish faktörlerin tersine çevrilmesiyle üretilir.
 * RSI ≤ 25 = max bear sinyali, RSI > 55 = bear zayıf
 */
export function calculateBearScore(data: StockData): BearScoreResult {
  // RSI tersine çevrilmiş skor (7-gün ağırlıklı blend)
  const rsiBlend = data.rsi7 !== undefined ? data.rsi7 : data.rsi;
  let rsiBearScore: number;
  if (rsiBlend <= 25) rsiBearScore = 100;
  else if (rsiBlend <= 35) rsiBearScore = 75;
  else if (rsiBlend <= 45) rsiBearScore = 50;
  else if (rsiBlend <= 55) rsiBearScore = 20;
  else rsiBearScore = 0;

  // VWAP altında = bear için pozitif
  const vwapBearScore = data.price < data.vwap
    ? Math.min(Math.abs(data.vwap - data.price) / Math.max(data.atr, 0.01) * 20, 100)
    : 0;

  // RVOL: Yüksek hacim + negatif fiyat değişimi = bear güçlenmesi
  const rvolBearScore = data.rvol >= 1.5 && data.priceChange < 0
    ? rvolScore(data.rvol)
    : data.rvol * 20;

  // Yapı: Lower Lows / Lower Highs
  const structureBearScore = 100 - checkStructure(data.recentHighs || [], data.recentLows || []);

  // Gap down kalitesi
  const gapBearScore = data.gap < -0.5 ? gapScore(Math.abs(data.gap)) : 0;

  // Negatif fiyat değişimi
  const pcBearScore = data.priceChange < 0
    ? Math.min(Math.abs(data.priceChange) * 15, 100)
    : 0;

  // Velocity aşağı yön
  const velDirBearScore = data.velocityDir !== undefined && data.velocityDir < 0
    ? Math.abs(data.velocityDir) * 100
    : 0;

  // Zayıf kapanış = bear retention
  const retentionBearScore = data.closeVsHighPercent !== undefined && data.closeVsHighPercent < 0.3
    ? (1 - data.closeVsHighPercent) * 100
    : 0;

  // Volatilite
  const velVolScore = velocityScore(data).vol;

  // MarketCap
  const mcapScore = marketCapScore(data.marketCap);

  const factors: Record<string, number> = {
    rsi: rsiBearScore,
    vwap: Math.min(vwapBearScore, 100),
    rvol: Math.min(rvolBearScore, 100),
    structure: structureBearScore,
    gap: gapBearScore,
    priceChange: pcBearScore,
    velocityDir: Math.min(velDirBearScore, 100),
    retention: Math.min(retentionBearScore, 100),
    velocityVol: velVolScore,
    marketCap: mcapScore,
  };

  // Ağırlıklar (bullish toplamıyla örtüşüyor)
  const weights: Record<string, number> = {
    rsi: 0.13, vwap: 0.13, rvol: 0.12, structure: 0.10,
    gap: 0.10, priceChange: 0.09, velocityDir: 0.07,
    retention: 0.08, velocityVol: 0.03, marketCap: 0.04,
  };

  const rawScore = Object.keys(factors).reduce((sum, k) =>
    sum + (factors[k] * (weights[k] || 0)), 0
  );

  const totalWeight = Object.values(weights).reduce((a, b) => a + b, 0);
  const normalizedScore = totalWeight > 0 ? (rawScore / totalWeight) * 100 : 50;
  const finalScore = clampScore(normalizedScore);

  // Bear sinyal threshold
  let signal: BearScoreResult["signal"];
  if (finalScore >= 75) signal = "STRONG_SELL";
  else if (finalScore >= 60) signal = "SELL";
  else if (finalScore >= 45) signal = "NEUTRAL";
  else signal = "AVOID";

  return { score: finalScore, signal, factors };
}

/**
 * T+1 Persistence Skoru — Hangi faktörler ertesi gün geçerli?
 *
 * T+1'de GEÇERLİ:  Structure, RSI, RVOL, MarketCap, Price Change
 * T+1'de GEÇERSİZ: ORB (intraday), VWAP (günlük reset), Retention (gün içi)
 *
 * Threshold: ≥ 65 = T+1 uygun
 */
export function calculatePersistenceScore(
  ...
  // T+1'de geçerli faktörler
  const structureScore = checkStructure(data.recentHighs || [], data.recentLows || []);
  const rsiVal = data.rsi7 !== undefined ? data.rsi7 : data.rsi;
  const rvolVal = data.rvol;
  const priceChgVal = Math.abs(data.priceChange);
  const mcapVal = marketCapScore(data.marketCap);
  ...
  if (direction === "BULL") {
    score =
      structureScore * 0.35 +
      (rsiVal > 50 && rsiVal < 75 ? (rsiVal - 50) * 2 : 0) * 0.25 +
      Math.min(rvolVal / 3 * 100, 100) * 0.20 +
      Math.min(priceChgVal * 10, 100) * 0.10 +
      mcapVal * 0.10;
  } else {
    score =
      (100 - structureScore) * 0.35 +
      (rsiVal < 50 && rsiVal > 25 ? (50 - rsiVal) * 2 : 0) * 0.25 +
      Math.min(rvolVal / 3 * 100, 100) * 0.20 +
      Math.min(priceChgVal * 10, 100) * 0.10 +
      mcapVal * 0.10;
  }
  ...
  const isT1Suitable = finalScore >= 65;

  return {
    score: finalScore,
    isT1Suitable,
    note: isT1Suitable
      ? `T+1 uygun: Trend, RSI, hacim tutarlı (${finalScore}/100)`
      : `T+1 zayıf: ORB/VWAP faktörleri ertesi gün geçersiz (${finalScore}/100)`,
  };
}

// ═══════════════════════════════════════════════════════════════════════════════
// v4.3: AI CATALYZER + MICROSTRUCTURE (PDF'den entegre)
// ═══════════════════════════════════════════════════════════════════════════════

import { analyzeCatalyst, integrateCatalyst } from "./aiCatalystAnalyzer";
import { checkMicrostructureDaily, integrateMicrostructure } from "./microstructureCheck";
import type { CatalystResult, MicrostructureResult } from "./aiCatalystAnalyzer";

/**
 * v4.3 Ana analiz fonksiyonu — Tüm PDF özellikleri birleşik
 * 1. Momentum skoru (11 faktör)
 * 2. Bear skoru (çift yönlü)
 * 3. Persistence skoru (PDT)
 * 4. AI Catalyst analizi (haber/katalizör)
 * 5. Microstructure check (yorgunluk tespiti)
 * 6. RSI RED filtresi
 */
export function analyzeStockFull(data: StockData): StockResult {
  // 1. Momentum skoru (mevcut)
  const result = analyzeStock(data);
  if (!result) return result as unknown as StockResult;

  const r = result as unknown as StockResult;

  // 2. Bear skoru
  const bearResult = calculateBearScore(data);
  r.bearScore = bearResult.score;
  r.bearSignal = bearResult.signal;

  // 3. Persistence skoru
  const persistence = calculatePersistenceScore(data, r.score > 50 ? "BULL" : "BEAR");
  r.persistenceScore = persistence.score;
  r.isT1Suitable = persistence.isT1Suitable;

  // 4. AI Catalyst analizi (PDF'den)
  // Gerçek uygulamada: web search API'den haber başlıkları çekilecek
  // Şimdilik: hisse adı + ticker bazlı keyword analizi
  const catalyst = analyzeCatalyst(r.ticker, []); // Boş array = nötr
  r.catalystScore = catalyst.score;
  r.catalystFlags = catalyst.flags;
  r.catalystSummary = catalyst.summary;

  // Katalizör skorunu entegre et
  if (catalyst.flags.length > 0) {
    r.score = integrateCatalyst(r.score, catalyst);
    // Risk flag varsa sinyal zayıflat
    if (catalyst.flags.includes("DILUTION") || catalyst.flags.includes("SEC_INVESTIGATION")) {
      if (r.signal === "STRONG_BUY") r.signal = "BUY";
      else if (r.signal === "BUY") r.signal = "NEUTRAL_BULLISH";
    }
  }

  // 5. Microstructure check (PDF'den)
  const micro = checkMicrostructureDaily(
    data.high || [],
    data.low || [],
    data.close || [],
    data.volume || [],
  );
  r.microScore = micro.microScore;
  r.microReversalRisk = micro.reversalRisk;
  r.microWarning = micro.warning;

  // Micro yapıyı entegre et
  const integrated = integrateMicrostructure(r.signal, r.score, micro);
  r.signal = integrated.signal;
  r.score = integrated.score;
  if (integrated.warning) {
    r.microWarning = integrated.warning;
  }
