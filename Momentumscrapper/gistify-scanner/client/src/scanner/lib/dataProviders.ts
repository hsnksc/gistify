...
const ALPHAVANTAGE_API_KEY = "d7mjhe1r01qngrvoel6gd7mjhe1r01qngrvoel70";
const TWELVEDATA_API_KEY = "d2bff39c345a49f1aa9ab15c70fcd6a9";
const MASSIVE_API_KEY = "3AjVXfyR6gxZgB9g3SpN7e6e7JLiIxTq";
...
export function getProviderStatuses(): ProviderStatus[] {
  ...
  return Object.entries(RATE_LIMITS).map(([name, limits]) => {
    ...
    return {
      ...
      active: !state.isBlocked && state.callsThisMinute < limits.perMinute,
...
export async function fetchStockDataHybrid(ticker: string): Promise<HybridFetchResult> {
  ...
  // 2. Massive dene (TEST EDILDI - 40 gun AAPL verisi ✅)
  try {
  ...
  // 3. Twelve Data dene (TEST EDILDI - calisiyor ✅)
  try {
